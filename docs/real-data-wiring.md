...
| --- | --- | --- | --- |
| OpenTopography DEM samples | `public/data/processed/opentopography-dem/samples.geojson` | `Point` | `elevation` or `elev` or `height` or `z` |
| OpenTopography contour lines | `public/data/processed/opentopography-contours/contours.geojson` | `LineString` | `elevation`, `interval`, `sourceId`, `sourceVersion`, `generatedAt` |
...
| LOD2-DE / federal-state shards | `public/data/processed/lod2-deutschland/index.json` or `public/data/processed/lod2-federal-states/index.json` | `Polygon` / `MultiPolygon` | CityGML-derived height/roof/source/state metadata |
...
- OpenTopography DEM point samples are sampled along the drawn M-scale section line.
- OpenTopography-derived contour lines are the preferred visual terrain layer.
- SRTM WMS is intentionally not used in the viewer because the public layer contains visual watermarking and is not an analysis source.
...
| --- | --- | --- |
| OpenTopography DEM API | Preprocessed DEM clips, samples, and contours | DEM clips can produce numeric samples and clean vector contours for analysis/export. |
| SRTM 30m WMS | Not used | The public WMS is display-only, watermarked, and unsuitable as a clean analysis/viewer layer. |
